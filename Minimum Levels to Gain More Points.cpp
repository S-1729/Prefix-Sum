...
public:
    int minimumLevels(vector<int>& possible) {
        long long sum=accumulate(begin(possible),end(possible),(long long)0);
        long long c0=count(begin(possible),end(possible),0);
        sum=sum-c0;
        long long Danielchandg=0,n=possible.size();
        for(int i=0;i<n-1;i++){
            if(possible[i]==1){
                Danielchandg++;
                sum--;
